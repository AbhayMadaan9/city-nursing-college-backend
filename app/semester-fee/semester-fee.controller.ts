...
export const validateSemesterFeeRequest = async (req: Request) => {
  ...
  const course = await courseService.getCourseByIdWithSemesters(associatedCourseId as string);
  if (!course) {
    throw new Error("Associated course not found");
  }

  // Check semester number
  const semesterNumberForComp = existingSemester ? existingSemester.semesterNumber : semesterNumber
  if (course.duration < semesterNumberForComp) {
    throw new Error(`Semester number ${semesterNumberForComp} exceeds the course duration of ${course.duration}`);
  }
  // Check course status
  ...
  if (semesterNumber) {
    ...
    if (isSemesterExists) {
      throw new Error(`Semester ${semesterNumber} already exists for course ${course.name}`);
    }
  }
...
export const createsemesterFee = asyncHandler(async (req: Request, res: Response) => {
  ...
  const updatedCourse = await courseService.editCourse(validatedCourse._id, {
    semesters: validatedCourse.semesters.map(semester => new Types.ObjectId(semester._id)),
  });
  if (!updatedCourse) {
  ...
  if (updatedCourse.semesters.length === validatedCourse.duration) {
    await courseService.editCourse(validatedCourse._id, { status: CourseStatus.COMPLETED });
  }

  res.send(createResponse(newSemesterFee, "Semester fee created successfully"));
});
...
export const updatesemesterFee = asyncHandler(async (req: Request, res: Response) => {
  ...
  const result = await semesterFeeService.updatesemesterFee(existingSemester._id, req.body);
...
export const editsemesterFee = asyncHandler(async (req: Request, res: Response) => {
  ...
  // Perform the edit
  const result = await semesterFeeService.editsemesterFee(existingSemester._id, req.body);
