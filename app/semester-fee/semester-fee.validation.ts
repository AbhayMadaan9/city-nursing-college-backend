        ...
        .custom((fees) => {
            fees.forEach((fee: any, feeIndex: number) => {
                ...
                if (!fee.type || typeof fee.type !== "string") {
                    throw new Error(`Fee at index ${feeIndex} must have a valid 'type' field`);
                }
                ...
                fee.details.forEach((detail: any, detailIndex: number) => {
                    ...
                    if (!detail.amount || typeof detail.amount !== "number" || detail.amount < 0) {
                        throw new Error(
...
export const createsemesterFee = [
    ...
    body("semesterNumber")
        .isInt({ min: 1 })
        .withMessage("Semester number must be an integer greater than or equal to 1"),
    // Validate fees array
    validateFees(),
];
